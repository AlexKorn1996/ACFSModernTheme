        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 md:py-24 grid grid-cols-1 md:grid-cols-2 gap-12 items-center">
            ...
            <div class="space-y-6">
                ...
                <p class="text-lg text-slate-500 leading-relaxed max-w-lg">
                    {$additionalHomeContent|strip_tags|truncate:250}
                ...
                <div class="flex flex-wrap gap-4 pt-2">
                    {if $issue}
                        ...
                        {/if}
                        <a href="{url page='issue' op='view' path=$issueId}" class="bg-brand-accent hover:bg-yellow-600 text-white px-6 py-3 rounded text-sm font-semibold transition flex items-center gap-2">
                ...
                <div class="relative bg-white p-6 rounded-xl shadow-soft border border-slate-100 max-w-sm ml-auto rotate-1 hover:rotate-0 transition duration-500">
                    <div class="flex justify-between items-start mb-4">
                        ...
                        {if $issue}
                            ...
                            {/if}
                            <span class="bg-brand-light text-brand-900 text-[10px] px-2 py-0.5 rounded font-bold">
                    ...
                    <h3 class="font-serif text-xl font-bold text-slate-900 mb-3">
                        {if $issue}
                            {if is_object($issue) && method_exists($issue, 'getLocalizedTitle')}
                                {$issue->getLocalizedTitle()|escape}
                            {elseif isset($issue.title)}
                                {$issue.title|escape}
                            {/if}
                        {else}ACFS Journal{/if}
                    </h3>
                    <p class="text-slate-500 text-sm mb-4">
                        {if $issue}
                            {if is_object($issue) && method_exists($issue, 'getLocalizedDescription')}
                                {$issue->getLocalizedDescription()|strip_tags|truncate:120}
                            {elseif isset($issue.description)}
                                {$issue.description|strip_tags|truncate:120}
                            {/if}
                        {/if}
                    </p>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex flex-col lg:flex-row gap-12">
                ...
                <div class="lg:w-2/3 space-y-8">
                    ...
                    {* List of articles in current issue *}
                    {if $issue}
                        <div class="sections">
